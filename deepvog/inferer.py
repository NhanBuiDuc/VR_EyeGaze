...
class nvgaze_inferer(object):
    def __init__(self, model, filter_model, flen, ori_video_shape, sensor_size, subject_number, eye, infer_gaze_flag=True):
        """
        Initialize necessary parameters and load deep_learning model
        ...
        self.imagefile = filtered_df['imagefile'].to_list()
        self.label_gaze_x = filtered_df['gaze_x'].to_list()
        self.label_gaze_y = filtered_df['gaze_y'].to_list()
    ...
    def process(self, video_src, mode, data, idx, model_name = None, keyword="", output_record_path="",
                batch_size=1, output_video_path="", 
                heatmap=False, print_prefix=""):
        """
        ...
        # framenames = []
        # # frame_len = len(glob.glob(video_src + "/*.jpg"))
        # # for i in range(1, 10+1):
        # #     framenames.append(video_src + "/%06d.jpg" % i)
        # for filename in self.imagefile:
        #     framenames.append(video_src + "/" + filename)  

        X_batch = np.zeros((1, vid_h, vid_w, 3))
        framename = video_src + "/" + data
        frame = cv2.imread(framename)
        frame_preprocessed = self._preprocess_image(frame, vid_h, vid_w)
        X_batch[0, :, :, :] = frame_preprocessed
        ...
        if mode == "Fit":
            self._fitting_batch(X_batch=X_batch,
                                Y_batch=Y_batch)
            # path = model_name + "/" + str(idx + 1)
            # self.save_eyeball_model(path)
        elif mode == "Infer":
            _, _, _ = self._infer_batch(X_batch=X_batch,
                                        Y_batch=Y_batch,
                                        idx=idx)
